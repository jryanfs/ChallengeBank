...
public sealed class TransferFlowIntegrationTests(MicroservicesFixture fixture)
    : IntegrationTestBase(fixture, useTransactionsApi: true)
{
    [Fact]
    public async Task CreateTransfer_AndGetById_AsAdmin_ReturnsCompletedTransferEnvelope()
    {
        ...
        var senderId = await CreateClientAsync(token, "11111111111", "sender@challengebank.com", "Sender");
        var receiverId = await CreateClientAsync(token, "22222222222", "receiver@challengebank.com", "Receiver");
    ...
    public async Task CreateTransfer_WithNullSenderUserId_ReturnsBadRequestInPortuguese()
    {
        ...
        var clientId = await CreateClientAsync(token, "88877766655", "receiver@challengebank.com", "Receiver");
    ...
    [Fact]
    public async Task CreateTransfer_WithEmptyReceiverUserId_ReturnsBadRequestInPortuguese()
    {
        var token = await LoginAsAdminAsync();
        SetBearerToken(token);

        var senderId = await CreateClientAsync(token, "10101010101", "onlyone@challengebank.com", "Only One");
        ...
        envelope.Message.Should().NotContain("could not be converted");
    }
    ...
    public async Task CreateTransfer_WithNonExistentSender_ReturnsNotFoundInPortuguese()
    {
        ...
        var receiverId = await CreateClientAsync(token, "77766655544", "onlyreceiver@challengebank.com", "Only Receiver");
    ...
    public async Task CreateTransfer_WithNonExistentReceiver_ReturnsNotFoundInPortuguese()
    {
        ...
        var senderId = await CreateClientAsync(token, "66655544433", "onlysender@challengebank.com", "Only Sender");
    ...
    public async Task CreateTransfer_WithSameSenderAndReceiver_ReturnsBadRequestInPortuguese()
    {
        ...
        var clientId = await CreateClientAsync(token, "99988877766", "same@challengebank.com", "Same User");
    ...
    public async Task ListTransfersByUser_AsUser_ReturnsForbidden_Admin_ReturnsOkEnvelope()
    {
        ...
        var clientId = await CreateClientAsync(adminToken, "33333333333", "list@challengebank.com", "List User");
    ...
    private async Task<Guid> CreateClientAsync(string token, string document, string email, string name)
    {
        SetBearerToken(token, ClientsClient);

        var response = await ClientsClient.PostAsJsonAsync("/api/clients", new
        {
            fullName = name,
